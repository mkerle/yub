...
Yub.prototype.verifyWithYubicoViaProxy = function (params, proxy, proxy_port, username, password, callback, currentTry) {
  ...
  https.get(options, function(res) {
    ...
    if (shouldRetry && serverError) {
      // Errored, but retry
      return me.verifyWithYubicoViaProxy(params, proxy, proxy_port, username, password, callback, currentTry + 1);
    } else if (badStatus) {
...
Yub.prototype.verify = function (otp, proxy, proxy_port, username, password, callback) {
  var me = this;
  this.calculateParams(otp, function (err, params) {
    if (err) return callback(err);

    me.verifyWithYubicoViaProxy(params, proxy, proxy_port, username, password, callback);
  });
};
...
Yub.verify = function(otp, proxy, proxy_port, username, password, callback) {
  if (!legacyInstance) throw new Error('init() before verifying!');
  return legacyInstance.verify(otp, proxy, proxy_port, username, password, callback);
};
